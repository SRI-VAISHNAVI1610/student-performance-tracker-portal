...
const Students = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const studentRes = await axios.get(`http://localhost:5000/api/students?${query}`, config);
    ...
    const handleAddStudent = async (e) => {
        ...
        try {
            const config = { headers: { Authorization: `Bearer ${user.token}` } };
            await axios.post('http://localhost:5000/api/students/admin/create-student', formData, config);
            setShowModal(false);
            fetchData();
            // Reset form
            setFormData({
