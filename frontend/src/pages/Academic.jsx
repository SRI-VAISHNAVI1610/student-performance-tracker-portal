    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                if (role === 'parent') {
                    const studentRes = await API.get(`/api/students?parentId=${user._id}`, config);
                    if (studentRes.data.length > 0) {
                        studentId = studentRes.data[0].userId;
                        setStudentInfo(studentRes.data[0]);
                    }
                } else if (role === 'student') {
                    const infoRes = await API.get(`/api/students/me`, config);
                    setStudentInfo(infoRes.data);
                }

                const marksRes = await API.get(`/api/marks?studentId=${studentId}`, config);
