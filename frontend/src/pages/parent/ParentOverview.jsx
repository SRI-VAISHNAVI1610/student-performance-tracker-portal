    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            const student = await API.get(
                `/api/students/profile?childId=${selected}`,
                config
            );
            ...
            const [marks, attendance, activities] = await Promise.all([
                API.get(`/api/marks?rollNumber=${roll}`, config),
                API.get(`/api/attendance?rollNumber=${roll}`, config),
                API.get(`/api/activities/student/${roll}`, config)
            ]);
