...
const Activities = () => {
    ...
    const fetchData = useCallback(async () => {
        try {
            ...
            if (isTeacher) {
                const res = await axios.get('http://localhost:5000/api/activity-requests/pending', config);
                ...
                const [reqRes, actRes] = await Promise.all([
                    axios.get(`http://localhost:5000/api/activity-requests/me`, config),
                    myProfile ? axios.get(`http://localhost:5000/api/activities/student/${myProfile.studentId}`, config) : Promise.resolve({ data: [] })
                ]);
                ...
                if (childProfile) {
                    const actRes = await axios.get(`http://localhost:5000/api/activities/student/${childProfile.studentId}`, config);
                    setActivities(actRes.data);
                }
            }
        } catch (err) {
    ...
    const handleRequestSubmit = async (e) => {
        ...
        try {
            const config = { headers: { Authorization: `Bearer ${user.token}` } };
            await axios.post('http://localhost:5000/api/activity-requests', formData, config);
            setShowForm(false);
            fetchData();
        } catch (err) {
    ...
    const handleUpdateStatus = async (requestId, status) => {
        try {
            ...
            await axios.put(`http://localhost:5000/api/activity-requests/${requestId}`, { status, points }, config);
            fetchData();
        } catch (err) {
