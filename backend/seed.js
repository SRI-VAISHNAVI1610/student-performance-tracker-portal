    ...
    try {
        ...
        for (const s of studentData) {
            ...
            for (const sub of subjects) {
                // Marks
                const internal1 = s.marks.int1 || 0;
                const internal2 = s.marks.int2 || 0;
                const semExam = s.marks.sem || 0;
                const total = internal1 + internal2 + semExam;
                const average = Number((total / 3).toFixed(2));

                await Mark.create({
                    rollNumber: s.rollNumber.trim().toUpperCase(),
                    subjectId: sub._id,
                    semester: s.sem, // added semester properly based on s.sem
                    internal1: internal1,
                    internal2: internal2,
